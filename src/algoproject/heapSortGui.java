 ...
 * @author Dell
 */
public class heapSortGui extends javax.swing.JFrame {
    int myArray[];
    String iteration;
    String resultt;
    StringBuilder s1;
    ...
    {
        int i,l,r;
        for(i=1;i<(myArray.length-1)/2;i++)
        {
            l=i*2;
            r=i*2+1;
            if(myArray[i]<myArray[l])
                swap(i,l);
            if(myArray[i]<myArray[r])
                swap(i,r);
        }
    }    
    public void maxHeapify(int pss)
    {
        int par=pss/2;
        
        if(par>0)
        {
            if(myArray[pss]>myArray[par])
                swap(pss,par);
            maxHeapify(par);
        }
        else 
            ...
            for(j=1;j<size-1/2;j++)
            {
                ...
                if(l<=size)
                    if(myArray[j]<myArray[l])
                        swap(j,l);
                if(r<=size)
                    if(myArray[j]<myArray[r])
                        swap(j,r);
            }
            for(int m=1;m<myArray.length;m++){
                s1.append(myArray[m]);
                if(m!=myArray.length-1)
                    s1.append(',');
            }
            s1.append('\n');            
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        for(int i=0;i<arr.length();i++){
            if(arr.charAt(i)==','){
                ...
                myArray[ps]=Integer.parseInt(temp);
                this.maxHeapify(ps);
                last=i+1;
                k++;
                ps++;
...
//        maxHeapifyDown();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(heapSortGui.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new heapSortGui().setVisible(true);
            }
        });
    }
